...
namespace GoNet_Comarch_SyncService.Services
{
    public class ErpApiClient : IErpApiClient
    {
        ...
        public int CreateClient(int sessionId, Client client)
        {
            ...
            {
                Wersja = _settings.ApiVersion,
                Akronim = client.Acronym.Trim(),
                Nazwa1 = SubstringSafe(client.Name, 0, 51).Trim(),
                Nazwa2 = SubstringSafe(client.Name, 51, 51).Trim(),
                Nazwa3 = SubstringSafe(client.Name, 102, 251).Trim(),
                NipPrefiks = client.Address.Country.Trim(),
                NipE = client.NIP.Trim(),
                Regon = client.Regon.Trim(),
                Opis = client.Description.Trim(),
                EMail = client.Email.Trim(),
                Telefon1 = client.Phone1.Trim(),
                Telefon2 = client.Phone2.Trim(),
                Cena = client.Price,
                Miasto = client.Address.City.Trim(),
                Ulica = (client.Address.Street + " " + client.Address.HomeNo).Trim(),
                KodP = client.Address.PostalCode.Trim(),
                Kraj = client.Address.Country.Trim(),
                ExpoKraj = client.Address.ExpCountry,
            };
        ...
        public int CreateClientBranch(int sessionId, ClientBranch branch)
        {
            ...
            XLAdresInfo_20241 xlBranch = new()
            {
                ...
                Akronim = branch.Acronym.Trim(),
                Nazwa1 = SubstringSafe(branch.Name, 0, 51).Trim(),
                Nazwa2 = SubstringSafe(branch.Name, 51, 51).Trim(),
                Nazwa3 = SubstringSafe(branch.Name, 102, 251).Trim(),
                NipPrefiks = branch.Address.Country.Trim(),
                NipE = branch.NIP.Trim(),
                Regon = branch.Regon.Trim(),
                EMail = branch.Email.Trim(),
                Telefon1 = branch.Phone1.Trim(),
                Telefon2 = branch.Phone2.Trim(),
                Miasto = branch.Address.City.Trim(),
                Ulica = (branch.Address.Street + " " + branch.Address.HomeNo).Trim(),
                KodP = branch.Address.PostalCode.Trim(),
                Kraj = branch.Address.Country.Trim(),
                AdresBank = 1
            };
